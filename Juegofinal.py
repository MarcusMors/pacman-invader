...
def game():
    ...
    while on:
        ...
        if keys[pygame.K_DOWN] and playersp.y + player_vel + playersp.get_height() + 15 < HEIGHT: # down
            playersp.y += player_vel
        if keys[pygame.K_SPACE] and state == 1:
            playersp.shoot()
        ...
        for enemy in enemies[:]:
            ...
            if random.randint(0, 2*FPS) == 1 and state == 1:
                enemy.shoot()
        ...
        if state == 1:
            ...
            surf = pygame.Surface((horizontal//2,vertical))
            pygame.draw.rect(surf, (150,150,150), surf.get_rect())
            surf.set_alpha(30)
            
            durf = pygame.Surface((horizontal//2,vertical))
            pygame.draw.rect(durf, (30,30,30), durf.get_rect())
            durf.set_alpha(255)

            screen.blit(surf,(0,0))
            screen.blit(durf,(horizontal//2,0))
            
            b_resume = buttons(horizontal//2 - 150,vertical//2 - 70, 300, 50,colour_b1, white, "Resume", screen, white, black, game)

            b_menu = buttons(horizontal//2 - 150,vertical//2 + 20, 300, 50,colour_b1, white, "Menu Principal", screen, white, black,main_menu)
