...
def init():
    global screen,size, horizontal, vertical, b_p, width_mid, lives,horizontalInical,horizontalFinal, verticalInical,verticalFinal,score
    pygame.init()
    score = 0
    horizontalInical=[]
    horizontalFinal=[]
    ...
    while on:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mixer.music.set_volume(0.3)
                on = False
        pacmanplayer.render(screen,(player.rect.x,player.rect.y),direccionaxis)
...
def ghost_move(ghost):
    ...
    #     ghost.move(-5, 0)
    #     ghost_counter += 1
    # elif ghost_movement == 5 and ghost_counter <= ghost_counter_limit:
    #     ghost.move( 5, 0)
    #     ghost_counter += 1
